...
| `RESEND_API_KEY`, `EMAIL_FROM`                                                                            | for email         | Without a key emails are printed to stdout (dry run) outside production; in production the `send-email` job fails with `email_not_configured` and retries until the key is set. |
