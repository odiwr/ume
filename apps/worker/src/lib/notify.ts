...
export async function notifyWorkspaceOwner(
  ...
  for (const u of managers) {
    ...
    if (status === 'failed')
      log.warn({ workspaceId: workspace.id, to, error }, `email ${input.kind} failed`)
    await record(db, {
  ...
  if (managers.length === 0 || seen.size === 0) {
    await record(db, {
      ...
      recipient: '-',
      status: 'skipped',
      error: 'no owner or manager with an email',
      metadata: input.metadata,
    })
  }
  ...
  if (owner?.discordUserId) {
    ...
    attempts.push({
      ...
      recipient: id,
      userId: owner.id,
      send: () => dmUser(log, id, input.discordText),
    })
  }
  if (workspace.noticeTextChannelId) {
    ...
    attempts.push({
      ...
      recipient: id,
      userId: null,
      send: () => postToChannel(log, id, input.discordText),
    })
  }
  attempts.push({
    channel: 'discord_channel',
    ...
    send: async () => {
      ...
      if (!guild?.systemChannelId)
        return { ok: false, reason: 'guild has no system channel (or bot is not in the guild)' }
  ...
  for (const a of attempts) {
    ...
    if (r.ok) break
    log.info(
      { workspaceId: workspace.id, channel: a.channel, reason: r.reason },
      `discord ${input.kind} attempt failed; trying next route`,
    )
  }
  if (!result.discord) {
    await record(db, {
