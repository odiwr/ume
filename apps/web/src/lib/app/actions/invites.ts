...
import { CAP, emailSchema, generateInviteToken, inviteCreateSchema, newId, roleGrantableByEmail, roleGrantableByLink } from '@ume/shared'
import { inviteEmail, sendEmail } from '@ume/email'
...
function assertGrantable(g: Guarded, caps: number): void {
  ...
  if ((caps & ~g.access.caps) !== 0) throw new AppError('You cannot invite people to a role with permissions you do not have.')
}

async function loadInviteRole(g: Guarded, roleId: string) {
  const role = await db.query.roles.findFirst({ where: and(eq(roles.id, roleId), eq(roles.workspaceId, g.workspace.id)) })
...
export async function createLinkInvite(workspaceId: string, input: CreateInviteInput): Promise<ActionResult<{ url: string; inviteId: string }>> {
  return runAction(async () => {
    const g = await guard(workspaceId, CAP.MANAGE_INVITES)
    const data = inviteCreateSchema.parse({ ...input, kind: 'link', membershipExpiresAt: input.membershipExpiresAt || null })
    if (data.maxUses !== null && data.maxUses > INVITE_MAX_USES) throw new AppError('That is too many uses.')
    const role = await loadInviteRole(g, data.roleId)
    if (!roleGrantableByLink(role.capabilities)) {
      throw new AppError(`"${role.name}" can delete or manage things, so it can only be granted by email invite. Share links are limited to contributor roles.`)
    }
    ...
    await logAudit(db, {
      ...
      targetType: 'invite',
      targetId: row!.id,
      metadata: { kind: 'link', roleId: role.id, maxUses: data.maxUses, expiresInDays: data.expiresInDays },
    })
...
export async function createEmailInvite(workspaceId: string, input: CreateInviteInput): Promise<ActionResult<{ inviteId: string; sent: boolean }>> {
  return runAction(async () => {
    const g = await guard(workspaceId, CAP.MANAGE_INVITES)
    const email = emailSchema.parse(input.email ?? '')
    const data = inviteCreateSchema.parse({ ...input, kind: 'email', email, maxUses: 1, membershipExpiresAt: input.membershipExpiresAt || null })
    const role = await loadInviteRole(g, data.roleId)
    if (!roleGrantableByEmail(role.capabilities)) throw new AppError('That role cannot be granted by invite.')
    const token = generateInviteToken()
    ...
    if (sent.ok) await db.update(invites).set({ emailSentAt: new Date() }).where(eq(invites.id, row!.id))
    await logAudit(db, {
      workspaceId,
      actorUserId: g.user.id,
      action: 'invite.create',
      targetType: 'invite',
      targetId: row!.id,
      metadata: { kind: 'email', roleId: role.id, email, sent: sent.ok },
    })
    revalidatePath(workspacePath(g.workspace.umeId), 'layout')
    return { inviteId: row!.id, sent: sent.ok }
  })
}

export async function revokeInvite(workspaceId: string, inviteId: string, removeMembers: boolean): Promise<ActionResult<{ removed: number }>> {
  return runAction(async () => {
    const g = await guard(workspaceId, CAP.MANAGE_INVITES)
    const invite = await db.query.invites.findFirst({ where: and(eq(invites.id, inviteId), eq(invites.workspaceId, workspaceId)) })
    ...
    if (removeMembers) {
      const where = g.workspace.ownerUserId
        ? and(eq(memberships.workspaceId, workspaceId), eq(memberships.inviteId, invite.id), ne(memberships.userId, g.workspace.ownerUserId))
...
export async function acceptInvite(token: string): Promise<ActionResult<string>> {
  const safeToken = z.string().regex(/^[a-z2-7]{16,64}$/).safeParse(token)
  const result = await runAction(async () => {
    ...
    if (invite.kind === 'email') {
      if (!me.email || me.email.toLowerCase() !== (invite.email ?? '').toLowerCase()) {
        throw new AppError(`This invite was sent to a different email address. Sign in with the account that received it.`)
      }
      if (!me.emailVerified) throw new AppError('Verify your email address with your sign-in provider before accepting this invite.')
    }
    if (invite.requireGuildMember) {
      if (!me.discordUserId) throw new AppError('This invite requires Discord. Link your Discord account and try again.')
      const member = await getGuildMember(invite.workspace.guildId, me.discordUserId).catch(() => null)
      if (!member) throw new AppError(`You need to be a member of the "${invite.workspace.guildName}" Discord server to accept this invite.`)
    }
    ...
    if (existing) {
      ...
      const keepRole = existing.role.systemKey === 'owner' || existing.role.position < invite.role.position
      await db
        .update(memberships)
        .set({
    ...
    await db.update(workspaces).set({ updatedAt: new Date() }).where(eq(workspaces.id, invite.workspaceId))
