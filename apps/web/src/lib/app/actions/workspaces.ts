...
import { playlists, can, claimToken, claimTokens, ensureDefaultRoles, getAccess, getWorkspaceByGuildId, logAudit, memberships, setWorkspaceOwner, workspaces, type ClaimError } from '@ume/db'
...
async function ensureDefaultPlaylist(workspaceId: string, userId: string): Promise<void> {
  ...
  await db
    .insert(playlists)
    .values({ id: newId('playlist'), workspaceId, name: 'Main', slug: slugify('Main'), emoji: '🎧', createdByUserId: userId })
    .onConflictDoNothing()
}
 ...
 */
export async function claimGuildViaOAuth(guildId: string): Promise<ActionResult<string>> {
  const result = await runAction(async () => {
    ...
    const now = new Date()
    let ws = await getWorkspaceByGuildId(db, guildId)
    const botGuild = await getGuild(guildId).catch(() => null)
    ...
    await logAudit(db, {
      ...
      targetId: ws.id,
      metadata: { via: 'oauth', guildId, becameOwner },
    })
...
export async function joinWorkspaceViaOAuth(guildId: string): Promise<ActionResult<string>> {
  const result = await runAction(async () => {
    ...
    if (!token) throw new AppError('Your Discord session expired. Sign in with Discord again.')
    const guilds = await fetchUserGuilds(token)
    if (!guilds.some((g) => g.id === guildId)) throw new AppError('You are not a member of that server.')
...
export async function submitClaimToken(_prev: ClaimTokenState, formData: FormData): Promise<ClaimTokenState> {
  ...
  await logAudit(db, {
    ...
    metadata: { via: 'token', becameOwner: result.becameOwner },
  })
  revalidatePath('/app', 'layout')
  redirect(returnTo.startsWith('/app/') ? returnTo : `/app/${result.workspace.umeId}`)
}
