 ...
 */
export function DataTable<T>({ columns, rows, rowKey, empty, className, dense }: DataTableProps<T>) {
  return (
      ...
      <table className="w-full min-w-max border-collapse text-sm">
        <thead>
          <tr className="border-b border-border bg-surface-2/60 text-left text-[11px] font-semibold uppercase tracking-wide text-fg-subtle">
            {columns.map((c) => (
              <th key={c.key} className={cn('px-3 py-2.5 whitespace-nowrap', c.align === 'right' && 'text-right', c.headerClassName)}>
                {c.header}
              </th>
            ))}
          </tr>
        ...
        <tbody>
          {rows.length === 0 ? (
            ...
            rows.map((row) => (
              <tr key={rowKey(row)} className="border-b border-border last:border-b-0 hover:bg-surface-2/50">
                {columns.map((c) => (
                  <td
                    key={c.key}
                    className={cn('px-3 align-middle', dense ? 'py-1.5' : 'py-2.5', c.align === 'right' && 'text-right tabular-nums', c.className)}
                  >
...
export function Mono({ children, className }: { children: React.ReactNode; className?: string }) {
  return <code className={cn('rounded bg-surface-3 px-1.5 py-0.5 font-mono text-[12px] text-fg-muted', className)}>{children}</code>
}
