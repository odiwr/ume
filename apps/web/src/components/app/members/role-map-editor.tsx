    ...
    <div className="space-y-4">
      ...
      >
        <div className="min-w-0">
          ...
          <p className="mt-1 text-xs text-fg-muted [text-wrap:pretty]">
            Members get the highest Ume role mapped to their Discord roles. Nobody is downgraded.
          </p>
        </div>
        <Switch
          checked={syncEnabled}
          disabled={pending}
          onCheckedChange={(v) =>
            run(() => setRoleSync(workspaceId, v), {
              success: v ? 'Role sync on.' : 'Role sync off.',
            })
          }
          aria-label="Apply Discord roles at sign-in"
        />
      </div>
      ...
      <div data-tinted="" className="rounded-2xl bg-surface-2 p-5">
        ...
        <Select
          ...
          disabled={pending}
          onChange={(e) =>
            run(() => setDefaultRole(workspaceId, e.target.value || null), {
              success: 'Default role updated.',
            })
          }
          className="max-w-sm"
        >
        ...
        <p className="mt-1.5 text-xs text-fg-muted [text-wrap:pretty]">
          For server members who match no mapping.
        </p>
      </div>

      <div data-tinted="" className="rounded-2xl bg-surface-2 py-2">
              ...
              >
                <div className="flex min-w-0 items-center gap-2">
                  <span
                    ...
                    style={{ background: dr.color ?? '#737373' }}
                    aria-hidden
                  />
                  <span className="truncate text-sm">
                    {dr.isEveryone ? '@everyone' : `@${dr.name}`}
                  </span>
                ...
                <Select
                  ...
                  onChange={(e) =>
                    run(
                      () =>
                        upsertRoleMap(workspaceId, {
                          discordRoleId: dr.id,
                          discordRoleName: dr.name,
                ...
                >
                  ...
                  {roles.map((r) => (
                    <option
                      key={r.id}
                      value={r.id}
                      disabled={!r.grantable && r.id !== dr.mappedRoleId}
                    >
                      {r.name}
                    </option>
                  ))}
