...
export function WorkspaceCard({
  ...
  return (
    <Link
      ...
      className="group flex flex-col gap-4 rounded-2xl bg-surface-2 p-5 transition-colors hover:bg-surface-3"
    >
      <div className="flex items-start gap-3">
        <GuildIcon
          name={workspace.guildName}
          src={guildIconUrl(workspace.guildId, workspace.guildIcon, 96)}
          size={44}
        />
        <div className="min-w-0 flex-1">
          ...
          <p className="mt-0.5 flex items-center gap-1.5 text-xs text-fg-muted">
            ...
            <span className="inline-flex items-center gap-1">
              <span
                className={
                  online ? 'size-1.5 rounded-full bg-success' : 'size-1.5 rounded-full bg-fg-subtle'
                }
                aria-hidden
              />
      ...
      <StorageBar
        ...
        quota={effectiveQuotaBytes(workspace)}
        compact
      />
    </Link>
  )
