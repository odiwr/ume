...
export function TopBar({
  ...
  return (
    <header className={cn('relative z-30 border-b border-border bg-bg', className)}>
        ...
        <div className="flex min-w-0 items-center gap-2">
          ...
          {crumbs?.length ? (
            <nav
              ...
              className="hidden min-w-0 items-center gap-1.5 text-sm text-fg-muted sm:flex"
            >
              {crumbs.map((c, i) => (
                <span key={c.label} className="flex items-center gap-1.5">
                  ...
                  {c.href ? (
                    ...
                    <span className={cn('truncate', i === crumbs.length - 1 ? 'text-fg' : '')}>
                      {c.label}
                    </span>
                  )}
                </span>
