...
import {
  acceptLinkAttestation,
  setLinkExtractEnabled,
  updateBotChannels,
} from '@/lib/app/actions/settings'
  ...
  return (
    <form
      ...
      onSubmit={(e) => {
        ...
        run(
          () =>
            updateBotChannels(workspaceId, {
              homeVoiceChannelId: home || null,
              noticeTextChannelId: notice || null,
            }),
          { success: 'Channels saved. The bot moves on its next heartbeat.' },
        )
      }}
    >
      <div>
...
export function MusicSettings({
  ...
  return (
    <div className="space-y-4">
      <div
        data-tinted=""
        className="flex items-start justify-between gap-4 rounded-2xl bg-surface-2 p-5"
      >
        <div className="min-w-0">
          <p className="font-medium">Add from link</p>
          <p className="mt-1 text-xs text-fg-muted [text-wrap:pretty]">
            Members with “Add music” can add a song from a YouTube, SoundCloud, Bandcamp, Audius,
            Mixcloud, Vimeo or Internet Archive link. When off, links are saved without audio.
          </p>
        </div>
        <Switch
          checked={enabled}
          disabled={pending}
          onCheckedChange={(v) =>
            run(() => setLinkExtractEnabled(workspaceId, v), {
              success: v
                ? 'Add from link is on.'
                : 'Add from link is off. Links are saved without audio.',
            })
          }
      ...
      {!globalFlagOn ? (
        <Notice tone="warning">
          Adding audio from links is switched off on Ume right now, for every server. Links are
          saved as metadata-only entries until it is back.
        </Notice>
      ) : null}
      ...
      <div data-tinted="" className="rounded-2xl bg-surface-2 p-5">
        <p className="flex items-center gap-2 font-medium">
          <ShieldCheck
            className={`size-4 ${acceptedAt ? 'text-success' : 'text-fg-muted'}`}
            aria-hidden
          />{' '}
          Rights attestation
        </p>
        {acceptedAt ? (
          <p className="mt-1 text-xs text-fg-muted [text-wrap:pretty]">
            Accepted{acceptedBy ? ` by ${acceptedBy}` : ''} on{' '}
            {new Date(acceptedAt).toLocaleDateString('en-US', {
              year: 'numeric',
              month: 'long',
              day: 'numeric',
            })}
            . Ume stores audio from links for this server.
          </p>
        ) : (
          <div className="mt-2 space-y-4">
            <p className="text-xs text-fg-muted [text-wrap:pretty]">
              Not accepted yet, so links are saved without audio.{' '}
              {isOwner
                ? 'Accepting it turns the extractor on for this server.'
                : 'Only the workspace Owner can accept it.'}
            </p>
            {isOwner ? (
              <>
                <CheckboxField
                  id="settings-attestation"
                  checked={attested}
                  onCheckedChange={(v) => setAttested(v === true)}
                  label={ATTESTATION_LABEL}
                  description="Takedown notices disable the track and block its content across Ume."
                />
                <Button
                  size="sm"
                  loading={pending}
                  disabled={!attested}
                  onClick={() =>
                    run(() => acceptLinkAttestation(workspaceId, attested), {
