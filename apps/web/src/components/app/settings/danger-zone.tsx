...
export function DangerZone({ workspaceId, guildName, memberCount, status }: { workspaceId: string; guildName: string; memberCount: number; status: string }) {
    ...
    <div className="divide-y divide-danger/20 rounded-2xl border border-danger/30 bg-danger/5">
      <div className="flex flex-col gap-3 p-5 sm:flex-row sm:items-center sm:justify-between">
        <div className="min-w-0">
          ...
          <p className="mt-1 text-xs text-fg-muted [text-wrap:pretty]">
            Removes every web member except you ({others} {others === 1 ? 'person' : 'people'}), revokes all invites and disconnects the workspace until a new token is entered. Playlists and music stay.
          </p>
        </div>
        <Button variant="danger" size="sm" onClick={() => setAction('reset')} disabled={purging} className="shrink-0">
      ...
      <div className="flex flex-col gap-3 p-5 sm:flex-row sm:items-center sm:justify-between">
        <div className="min-w-0">
          ...
          <p className="mt-1 text-xs text-fg-muted [text-wrap:pretty]">Deletes playlists, music, members, invites and settings. The bot leaves the workspace behind. There is no undo.</p>
        </div>
        <Button variant="danger" size="sm" onClick={() => setAction('purge')} disabled={purging} className="shrink-0">
            ...
            <form
              ...
              onSubmit={async (e) => {
                ...
                const res = action === 'reset'
                  ? await run(() => resetWorkspace(workspaceId, typed), { success: (d) => `Workspace reset. ${d.removed} member${d.removed === 1 ? '' : 's'} removed.` })
            ...
            >
              <div>
                <Label htmlFor="danger-typed">
                  Type <span className="normal-case tracking-normal text-fg">{guildName}</span> to confirm
                </Label>
                <Input id="danger-typed" value={typed} onChange={(e) => setTyped(e.target.value)} autoComplete="off" spellCheck={false} autoFocus placeholder={guildName} />
              </div>
              <DialogFooter>
                <Button type="button" variant="ghost" onClick={() => setAction(null)} disabled={pending}>
                  Cancel
                </Button>
                <Button type="submit" variant="danger" loading={pending} disabled={typed.trim() !== guildName.trim()}>
