  ...
  return (
    <>
      <Button
        ...
        disabled={full}
        title={full ? `Limit of ${PLAYLIST.maxPerWorkspace} playlists reached` : undefined}
      >
      ...
      <Dialog open={open} onOpenChange={(o) => (!pending ? setOpen(o) : undefined)}>
        <DialogContent
          title="New playlist"
          description="Playlists are flat: one level, any number of songs. You can rename it any time."
        >
          <form
            className="space-y-4"
            onSubmit={async (e) => {
              e.preventDefault()
              const res = await run(() => createPlaylist(workspaceId, { name }), {
...
export function PlaylistMenu({
  ...
  return (
    <>
      ...
      <Dialog
        open={dialog === 'rename'}
        onOpenChange={(o) => (!o && !pending ? setDialog(null) : undefined)}
      >
        <DialogContent
          title="Rename playlist"
          description="The link changes with the name; Discord commands use the new name right away."
        >
          <form
            className="space-y-4"
            onSubmit={async (e) => {
              e.preventDefault()
              const res = await run(
                () => renamePlaylist(workspaceId, playlist.id, { name, description }),
                { success: 'Playlist updated.' },
              )
              if (res.ok) {
                setDialog(null)
                if (res.data.slug !== playlist.slug && afterDelete !== 'library')
                  router.replace(`/app/${umeId}/library/${res.data.slug}`)
              }
            }}
          >
            <div>
            ...
            <div>
              ...
              <Textarea
                ...
                maxLength={300}
                placeholder="What belongs here, what does not."
              />
            </div>
            <DialogFooter>
              <Button
            ...
            <Button
              ...
              onClick={async () => {
                const res = await run(() => deletePlaylist(workspaceId, playlist.id), {
                  success: (d) =>
                    d.removedTracks
                      ? `Playlist deleted; ${d.removedTracks} song${d.removedTracks === 1 ? '' : 's'} removed from storage.`
                      : 'Playlist deleted.',
                })
                if (res.ok) {
                  setDialog(null)
