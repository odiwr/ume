...
export function AddLinkForm({
  ...
  const siteLabel = parsed
    ? (SUPPORTED_LINK_SITES.find((s) => s.site === parsed.site)?.label ?? 'Link')
    : null
  ...
  return (
    <form
      ...
      onSubmit={async (e) => {
        ...
        const res = await run(() => addLinkToPlaylist(workspaceId, playlistId, url), {
          success: (d) =>
            d.extracting ? `Added “${d.title}”. Ume is fetching the audio.` : `Added “${d.title}”.`,
        })
        if (res.ok) {
    ...
    >
      <div className="flex flex-col gap-2 sm:flex-row">
        <div className="relative min-w-0 flex-1">
          ...
          <Input
            ...
            className={`pl-9 ${invalid ? 'bg-danger/10 in-data-tinted:bg-danger/10' : ''}`}
          />
        </div>
      ...
      <p
        id="add-link-help"
        className={`text-xs [text-wrap:pretty] ${invalid ? 'text-danger' : 'text-fg-muted'}`}
      >
        {invalid
          ? 'That link is not a single track from a supported site. Playlists, albums and sets are not accepted.'
          : (extractionNotice ??
            `One link, one song. Supported: ${SUPPORTED_LINK_SITES.map((s) => s.label).join(', ')}.`)}
      </p>
    </form>
  )
}
