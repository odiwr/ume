...
export default async function CeoWorkspacesPage({ searchParams }: { searchParams: Promise<Record<string, string | string[] | undefined>> }) {
  ...
  const columns: Column<WorkspaceRow>[] = [
    {
      ...
      render: (r) => (
        <div className="min-w-0">
          <Link href={`/ceo/workspaces/${r.ws.id}`} className="font-medium text-fg hover:text-pink-soft">
            {r.ws.guildName}
          </Link>
    ...
    { key: 'status', header: 'Status', render: (r) => <WorkspaceStatusBadge status={r.ws.status} /> },
    ...
    { key: 'storage', header: 'Storage', align: 'right', render: (r) => formatBytes(r.ws.storageUsedBytes) },
    { key: 'tracks', header: 'Tracks', align: 'right', render: (r) => r.ws.trackCount.toLocaleString('en-US') },
  ...
  return (
    <>
      <PageHeader title="Workspaces" description="Every Discord server that has run /reload. Search by server name, Ume ID, guild ID or owner email." />
      <div className="flex flex-col gap-3">
        <SearchForm basePath="/ceo/workspaces" q={q} placeholder="Search name, ume-…, guild id, owner email" hidden={{ status }} />
        <FilterChips
          basePath="/ceo/workspaces"
          current={status ?? null}
          extra={{ q }}
          options={[{ value: null, label: 'All' }, ...WORKSPACE_STATUSES.map((s) => ({ value: s, label: s }))]}
        />
      </div>
      <DataTable columns={columns} rows={result.rows} rowKey={(r) => r.ws.id} empty="No workspaces match." />
      <Pagination page={result.page} pageSize={result.pageSize} total={result.total} basePath="/ceo/workspaces" params={{ q, status }} />
    </>
  )
}
