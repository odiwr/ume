...
import { Activity, Clock, Database, DollarSign, HardDrive, Music, Radio, Server, TriangleAlert, Users } from 'lucide-react'
...
export default async function CeoOverviewPage() {
  ...
  return (
    <>
      <PageHeader title="Overview" description="Where Ume stands right now. Numbers refresh on every load." />
      ...
      <StatGrid>
        ...
        />
        <StatTile label="Storage used" value={formatBytes(o.storageUsedBytes)} detail="Sum of every workspace's normalized Opus" icon={<HardDrive className="size-4" />} />
        <StatTile
          label="MRR"
          value={`$${n(o.mrrUsd)}`}
        ...
        <StatTile
          ...
          tone={o.workspaces.connected > 0 && o.workspaces.botOnline < o.workspaces.connected ? 'warning' : 'default'}
        />
        <StatTile
        ...
        <StatTile
          ...
          detail={
            o.jobs.available
              ? o.jobs.byState.map((s) => `${s.state} ${n(s.count)}`).join(' · ') || 'Queue is empty'
      ...
      <div className="grid grid-cols-1 gap-4 xl:grid-cols-2">
        <Card>
          <CardHeader>
            ...
            <CardDescription>Servers that met the bot for the first time, last 12 ISO weeks.</CardDescription>
          </CardHeader>
          <CardContent>
        ...
        <Card>
          <CardHeader>
            ...
            <CardDescription>Bytes stored per tier. Free is the long tail; paid tiers are the bill.</CardDescription>
          </CardHeader>
          <CardContent>
      ...
      <Section
        ...
        actions={
          <Link href="/ceo/workspaces?status=connected" className="text-sm text-fg-muted hover:text-fg">
            All workspaces
          </Link>
        }
      >
            ...
            <ul className="divide-y divide-border">
              {idle.map((w) => (
                <li key={w.id} className="flex flex-wrap items-center justify-between gap-2 px-5 py-3 text-sm">
                  ...
                  <Badge tone="warning">
                    <TriangleAlert className="size-3" /> last activity <Ago date={w.lastActivityAt} />
                  </Badge>
                </li>
              ))}
            </ul>
