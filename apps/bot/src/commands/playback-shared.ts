...
import { VoicePermissionError, voicePermissionEmbed } from '../voice/permissions'
  ...
  if (!v.isConnected(guild.id)) {
    if (!ws.homeVoiceChannelId) {
      await ctx.reply({
        embeds: [
          errorEmbed(
            'I have no home channel yet. Join a voice channel and run `/home`.',
            'No home channel',
          ),
        ],
      })
    ...
    if (!opts.joinIfNeeded) {
      await ctx.reply({
        embeds: [
          errorEmbed(
            `I am not connected to <#${ws.homeVoiceChannelId}> right now — reconnecting. Try again in a moment.`,
            'Not connected',
          ),
        ],
      })
      void v.backgroundJoin(guild, ws.homeVoiceChannelId, ws.id, 'reconnect from command failed')
      return null
    }
    try {
      ...
      await ctx.reply({
        embeds: [
          err instanceof VoicePermissionError
            ...
            : errorEmbed(
                `Could not join <#${ws.homeVoiceChannelId}>: ${(err as Error).message}`,
                'Could not join',
              ),
        ],
      })
...
export function addedByLabel(entry: {
...
export async function playlistQueue(
  playlist: Playlist,
): Promise<{ items: QueueItem[]; skippedLinked: number; notReady: number }> {
