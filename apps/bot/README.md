...
| `RESEND_API_KEY`, `EMAIL_FROM` | for `/reload` | Emails the previous Owner when a token rotation disconnects their workspace. Dry-run to stdout without a key. |
...
channel status line ("Now playing: …") additionally needs *Set Voice Channel Status* (or
