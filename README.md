...
| `STRIPE_PRICE_PLUS`, `STRIPE_PRICE_PRO`, `STRIPE_PRICE_STUDIO`, and the same with `_YEARLY` | web                                      | Stripe → Product catalog → the `price_…` id of each monthly and yearly price (plans in `packages/shared/src/plans.ts`).                                                                                                                                                                             |
| `RESEND_API_KEY`, `EMAIL_FROM`                                                              | web, bot, worker                         | Resend → API Keys, after verifying the sending domain. Without a key, emails are printed to stdout outside production; in production (`NODE_ENV=production`) sends fail with `email_not_configured` instead of pretending to succeed.                                                               |
